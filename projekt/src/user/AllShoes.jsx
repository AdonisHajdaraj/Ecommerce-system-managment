...
const AllShoes = () => {
  ...
  useEffect(() => {
    const fetchAllShoes = async () => {
      try {
        const [menResShoe, womenResShoe, kidsResShoe] = await Promise.all([
          axios.get('http://localhost:3002/shoes'),
          axios.get('http://localhost:3002/shoewomen'),
          axios.get('http://localhost:3002/shoeskids'),
        ]);

        const menWithTypeShoe = menResShoe.data.map(item => ({ ...item, type: 'shoe' }));
        const womenWithTypeShoe = womenResShoe.data.map(item => ({ ...item, type: 'shoewomen' }));
        const kidsWithTypeShoe = kidsResShoe.data.map(item => ({ ...item, type: 'shoekids' }));

        const allProducts = [...menWithTypeShoe, ...womenWithTypeShoe, ...kidsWithTypeShoe];
  ...
  useEffect(() => {
    ...
    const categories = ['shoe', 'shoewomen', 'shoekids'];
      ...
      <div className="container mt-4">
        ...
        <div className="d-flex justify-content-center mb-4 gap-3 flex-wrap">
          ...
          >
            ...
            <option value="shoe">🧑 Meshkuj</option>
            <option value="shoewomen">👩 Femra</option>
            <option value="shoekids">👶 Fëmijë</option>
