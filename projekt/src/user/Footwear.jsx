  ...
  useEffect(() => {
    const fetchAllFootwear = async () => {
      try {
        ...
        const menWithType = menRes.data.map(item => ({ ...item, type: 'shoes' }));
        const womenWithType = womenRes.data.map(item => ({ ...item, type: 'shoewomen' }));
        const kidsWithType = kidsRes.data.map(item => ({ ...item, type: 'shoeskids' }));
  ...
  return (
    <div className="d-flex min-vh-100" style={{ backgroundColor: '#F0F0F0' }}>
      ...
      <div className="container mt-4">
        <h1 className="mb-4 text-center">Footwear Collection</h1>
