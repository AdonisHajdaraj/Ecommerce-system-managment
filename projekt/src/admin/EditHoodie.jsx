...
const EditHoodie = () => {
  ...
  const hoodieId = location.pathname.split("/")[2];

  // Fetch existing hoodie data
  useEffect(() => {
    const fetchHoodie = async () => {
      try {
        const res = await axios.get(`http://localhost:3002/hoodie/${hoodieId}`);
        setHoodie({
          name: res.data.name,
  ...
  const handleChange = (e) => {
    setHoodie((prev) => ({ ...prev, [e.target.name]: e.target.value }));
  };
  ...
  const handleClick = async (e) => {
    ...
    formData.append("name", hoodie.name);
    formData.append("price", hoodie.price);
    if (hoodie.cover instanceof File) {
      formData.append("cover", hoodie.cover); // Append new file if changed
    }

    try {
      await axios.put(`http://localhost:3002/hoodie/${hoodieId}`, formData, {
